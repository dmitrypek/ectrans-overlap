MODULE OVERLAP_TYPES_MOD
  ...
  TYPE, PUBLIC :: BATCH
      ...
      INTEGER(KIND=JPIM) :: NSTRFS   ! INDEX TO THE FIRST DIMENSION OF SLICE OF ZGTF FOR THIS BATCH
...
CONTAINS
    ...
    FUNCTION BATCH_CONSTRUCTOR(KBLK, KF_GP, KF_SCALARS_G, KF_UV_G, KVSETUV, KVSETSC) RESULT(THIS)
        ...
        USE DIR_TRANS_CTL_MOD, ONLY: ZGTF
        ...
        IF(KBLK .EQ. 1) THEN
           NPTRFS(1) = 1
        ENDIF
        NPTRFS(KBLK+1) = NPTRFS(KBLK) + NF_FS
        THIS%NSTRFS = NPTRFS(KBLK)
    ...
    SUBROUTINE START_COMM(THIS, PGP)
      ...
      USE DIR_TRANS_CTL_MOD, ONLY : IREQ_RECV
        ...
        IF (THIS%NF_UV_G > 0 .AND. THIS%NF_SCALARS_G > 0) THEN
          CALL FTDIR_CTL_COMM_SEND(THIS%NSTRFS,THIS%NF_UV_G, THIS%NF_SCALARS_G, THIS%NF_GP, &
            &                      THIS%NF_FS, KVSETUV=THIS%NVSETUV(THIS%NSTUV_G:THIS%NENUV_G), &
            &                      KVSETSC=THIS%NVSETSC(THIS%NSTSC_G:THIS%NENSC_G), &
            &                      KPTRGP=THIS%NPTRGP, IREQ_RECV(:,THIS%NBLK),THIS%NBLK,PGP=PGP)
        ...
        ELSEIF (THIS%NF_SCALARS_G > 0) THEN
          CALL FTDIR_CTL_COMM_SEND(THIS%NSTRFS,THIS%NF_UV_G, THIS%NF_SCALARS_G, THIS%NF_GP, &
            &                      THIS%NF_FS, KVSETSC=THIS%NVSETSC(THIS%NSTSC_G:THIS%NENSC_G), &
            &                      KPTRGP=THIS%NPTRGP, IREQ_RECV(:,THIS%NBLK),THIS%NBLK,PGP=PGP)
        ENDIF
    ...
    SUBROUTINE FINISH_COMM(THIS, PSPVOR, PSPDIV, PSPSCALAR)
      ...
      CALL FTDIR_CTL_COMP(THIS%NF_FS,THIS%NSTRFS)
