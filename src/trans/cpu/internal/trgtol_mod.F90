...
SUBROUTINE TRGTOL(PGLAT,KF_FS,KF_GP,KF_SCALARS_G,KVSET,KPTRGP,IREQ_RECV,IBLK, &
...
USE TPM_DISTR ,ONLY : D, NPRTRNS, NPROC
USE TPM_TRANS ,ONLY : NGPBLKS
USE DIR_TRANS_CTL_MOD, ONLY : ZCOMBUFR_HEAP
...
CALL TRGTOL_COMM_SEND(PGLAT, KF_FS, KF_GP, KF_SCALARS_G, KVSET, KSENDCOUNT, KRECVCOUNT, KNSEND, &
  ...
  &                   ZCOMBUFS_HEAP, ZCOMBUFR_HEAP(1,1,IBLK), KPTRGP, PGP, PGPUV, PGP3A, PGP3B, PGP2)

CALL TRGTOL_COMM_RECV(PGLAT, KF_FS, KRECVCOUNT, KNRECV, KRECVTOT, KRECV, KINDEX, KNDOFF, &
  &                   ZCOMBUFR_HEAP(1,1,IBLK))
...
CALL TRGTOL_COMM_SEND(PGLAT, KF_FS, KF_GP, KF_SCALARS_G, KVSET, KSENDCOUNT, KRECVCOUNT, KNSEND, &
  &                   KNRECV, KSENDTOT, KRECVTOT, KSEND, KRECV, KINDEX, KNDOFF, KGPTRSEND, &
  &                   ZCOMBUFS_STACK, ZCOMBUFR_STACK, KPTRGP, PGP, PGPUV, PGP3A, PGP3B, PGP2)
...
!IF (.NOT. ALLOCATED(IREQ_RECV)) ALLOCATE(IREQ_RECV(NPROC))
...
FUNCTION TRGTOL_COMM_TEST() RESULT(LL_COMM_DONE)
  LOGICAL :: LL_COMM_DONE

  LL_COMM_DONE = .TRUE.

END FUNCTION TRGTOL_COMM_TEST

SUBROUTINE TRGTOL_COMM_RECV(PGLAT, KF_FS, KRECVCOUNT, KNRECV, KRECVTOT, KRECV, KINDEX, KNDOFF, PCOMBUFR)
...
REAL(KIND=JPRB),    INTENT(INOUT) :: PGLAT(KF_FS,D%NLENGTF)
...
INTEGER(KIND=JPIM) :: JNR, INR, IRECV, ILEN, IRECV_FLD_START, IRECV_FLD_END, JL, II, JFLD
